...
ggplot(data = vectorshare, aes(x = vectorshare$tweetcount, y = rownames(vectorshare))) + 
  geom_bar(stat = "identity", width = 0.5, position = "dodge", fill = "darkorange", color = "black") + 
  xlab("Year") +
  ylab("Number of likes")  +
  ggtitle("Likes per year") +
...
GRAFICO CHE CONTA QUANTITTA DI TWEET CHE CONTENGONO DETERMINATE KEYWORD CRYPTO-RELATED E PERCENTUALE SUL TOTALE DEI TWEET
...
plot(e)
```
DO THE SAME WITH A SUBSET CONSIDERING ONLY FROM A SPECIFIC POINT ONWARD AND WITHOUT AMP, compare and see if there is any difference.


## Subsetting sentiment from Jan 2020 without "AMP" word
...
mean(senti1$sentiment)
sd(senti1$sentiment)
skewness(senti1$sentiment)
